...
const getNeighbors = (coords: d.v2i, bounds: d.v2i): d.v2i[] => {
  ...
  for (const i of tgpu.unroll(std.range(4))) {
    adjacentOffsets[i] = std.clamp(coords.add(adjacentOffsets[i]), d.vec2i(), std.sub(bounds, d.vec2i(1)));
  }
...
export const brushFn = tgpu.computeFn({
  ...
  if (distSquared < radiusSquared) {
    ...
    forceVec = brushSettings.delta.mul(brushSettings.forceScale * brushWeight);
...
export const addForcesFn = tgpu.computeFn({
  ...
  const newVel = currentVel.add(forceVec.mul(timeStep));
...
export const advectFn = tgpu.computeFn({
  ...
  const prevPos = std.sub(d.vec2f(pixelPos), velocity.xy.mul(timeStep));
  const clampedPos = std.clamp(prevPos, d.vec2f(-0.5), std.sub(d.vec2f(texSize.xy), 0.5));
...
export const diffusionFn = tgpu.computeFn({
  ...
  const diffusedVal =
    d.vec4f(blendFactor).mul((leftVal.add(rightVal).add(upVal).add(downVal).add(centerVal.mul(diffuseRate))));
...
export const advectInkFn = tgpu.computeFn({
  ...
  const prevPos = std.sub(d.vec2f(pixelPos), velocity.mul(timeStep));
